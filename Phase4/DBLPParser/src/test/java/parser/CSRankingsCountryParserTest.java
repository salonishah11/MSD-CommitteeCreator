...
public class CSRankingsCountryParserTest {
	...
	public CSRankingsCountryParserTest() {
	...
	public void testParseCSRankingsData() {
		...
		boolean res = false;
		File csRankingsFile = new File("testdata/country-info.csv");
		res = parseXMLCompObj.parseCSRankingsUnivRegion(csRankingsFile);
	...
	public void testParseCSRankingsDataLoad() {
		...
		try {
			...
			stmnt.executeUpdate("delete from universitydetails where university='Ariel University'");
			File committeeDir = new File("testdata/country-info.csv");
			parseXMLCompObjDB.parseCSRankingsUnivRegion(committeeDir);
			ResultSet r = stmnt.executeQuery("select count(*) as cnt from universitydetails where university='Ariel University'");
			while (r.next()) {
