...
public class ArticleParserTest {
	...
	public void testParseXMLCheckArticlesRecords() {
		...
		try {
			...
			stmnt.executeUpdate("delete from journalpaper where keyval='journals/tse/GuptaAF91' and type='article'");
			stmnt.executeUpdate("delete from journalauthor where keyval='journals/tse/GuptaAF91'");
			File dblpXMLFile = new File("testdata/article_sample.xml");
			parseXMLCompObj.parseXML(dblpXMLFile);
			ResultSet r = stmnt.executeQuery(
					"select count(*) as cnt from journalpaper where type='article' and keyval='journals/tse/GuptaAF91'");
			while (r.next()) {
				artCnt = r.getInt("cnt");
			}
	...
	public void testParseXMLCheckArticlesRecordUnmappable() {
		...
		try {
			...
			stmnt.executeUpdate("delete from journalpaper where keyval='journals/test/tse/PauleveMR11' and type='article'");
			stmnt.executeUpdate("delete from journalauthor where keyval='journals/test/tse/PauleveMR11'");
			File dblpXMLFile = new File("testdata/article_unmappable.xml");
			parseXMLCompObj.parseXML(dblpXMLFile);
			ResultSet r = stmnt.executeQuery(
					"select count(*) as cnt from journalpaper where type='article' and keyval='journals/test/tse/PauleveMR11'");
			while (r.next()) {
				artCnt = r.getInt("cnt");
			}
			stmnt.executeUpdate("delete from journalpaper where keyval='journals/test/tse/PauleveMR11' and type='article'");
